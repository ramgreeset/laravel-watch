...
В LessonSeeder мы обращаемся к Course::each. Перебираем курсы и для каждого из них генерируем определенное кол-во уроков
так же связываем текущие сгенерированные уроки с конкретным курсом и добавляем в таблицу данных number курса.
</pre>>
</ul>
